...
// HACK:  Made this non-static to be able to call it from the main utilityblox mesh tool
int js_to_string(lua_State *L) {
    yajl_gen* gen;
...
static int js_generator_value(lua_State *L) {
    ...
    case LUA_TTHREAD:
		...
		// Functions get converted to their address as an int
		if( lua_isfunction( L, -1 ) ) {
			int address = (int) lua_topointer( L, -1 );
			lua_pop( L, 1 );
