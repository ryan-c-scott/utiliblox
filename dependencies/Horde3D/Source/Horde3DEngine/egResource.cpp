...
Resource::Resource( int type, const string &name, int flags )
{
    ...
    _hashedName = Data::Hash64( name.c_str() );
...
{	
	...
	if( data == 0x0 || size <= 0 )
	{	
		Modules::log().writeError( "Resource '%s' of type %i: No data loaded (file not found?)", _name.c_str(), _type );
		_noQuery = true;
		return false;
	}
...
ResourceManager::~ResourceManager()
{
	...
	eastl::map< int, ResourceRegEntry >::const_iterator itr = _registry.begin();
...
Resource *ResourceManager::findResource( const string &name )
{
    return findResource( Data::Hash64( name.c_str() ) );
}

Resource *ResourceManager::findResource( const uint64_t hashedName )
{
	for( size_t i = 0, s = _resources.size(); i < s; ++i )
	{
		if( _resources[i] != 0x0 && _resources[i]->_hashedName == hashedName )
		{
			return _resources[i];
		}
	}
...
ResHandle ResourceManager::addResource( int type, const string &name, int flags, bool userCall )
{
	...
	eastl::map< int, ResourceRegEntry >::iterator itr = _registry.find( type );
...
{
	eastl::vector< uint32 > killList;
	
	// Find unused resources and release dependencies
	for( uint32 i = 0; i < _resources.size(); ++i )
	{
		if( _resources[i] != 0x0 && _resources[i]->_refCount == 0 )
		{
	...
	for( uint32 i = 0; i < killList.size(); ++i )
	{
		...
		delete _resources[killList[i]];
        _resources[killList[i]] = 0x0;
	}
    ...
    void ResourceManager::killResource( int res )
    {
        int handle = res - 1;
        
        if( !_resources[handle] )
            return;

        _resources[handle]->release();
        delete _resources[handle];
        _resources[handle] = 0x0;
    }
